...
export async function handlecreateJiraIssue(req: Request, res: Response): Promise<void> {
    ...
    try {
        ...
        const { taskType, description } = await aiService.detectTaskTypeAndGenerateDescription(input);
        ...
        const answer = await jiraService.createJiraIssue(projectKey, taskType, description);
